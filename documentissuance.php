...
if (isset($_REQUEST['del'])) {
    $uid = intval($_GET['del']);
    $sql = "DELETE FROM documentissuance WHERE id=:id";
        ...
        <div class="main p-5">
            ...
            <div class="container mt-5">
                <table id="documenttable" class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT * FROM documentissuance";
                        $query = $dbh->prepare($sql);
                        $query->execute();
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlentities($result->Document); ?></td>
                                    <td><?php echo htmlentities($result->Amount); ?></td>
                                    <td><?php echo htmlentities($result->Referenceno); ?></td>
                                    <td><?php echo htmlentities($result->Date); ?></td>
                                    <td><?php echo htmlentities($result->Status); ?></td>
                                    <td>
                                        <a href="documentissuance.php?del=<?php echo htmlentities($result->id); ?>"><i class="lni lni-trash-can p-2" onclick="return confirm('Do you really want to delete?')"></i></a>
                                        <a href="updatedocumentissuance.php?id=<?php echo htmlentities($result->id); ?>"><i class="lni lni-pencil p-2"></i></a>
                                        <?php if ($result->Document == "BARANGAY RESIDENCY") { ?>
                                            <a href="certificateresidency.php?id=<?php echo htmlentities($result->id); ?>" target="_blank" onclick="var newWindow = window.open(this.href); newWindow.onload = function() { newWindow.print(); }; return false;">
                                                <i class="fas fa-file-pdf p-2"></i>
                                            </a>
                                        <?php } elseif ($result->Document == "BARANGAY INDIGENCE") { ?>
                                            <a href="certificateofindigence.php?id=<?php echo htmlentities($result->id); ?>" target="_blank" onclick="var newWindow = window.open(this.href); newWindow.onload = function() { newWindow.print(); }; return false;">
                                                <i class="fas fa-file-pdf p-2"></i>
                                            </a>
                                        <?php } else { ?>
                                            <i class="fas fa-file-pdf p-2" style="color: gray;"></i>
                                        <?php } ?>
                                    </td>
                                </tr>
                        <?php
